  ...
  return (
    <header className="sticky top-0 z-30 border-b border-line bg-page/95 backdrop-blur-md">
      <div className="mx-auto flex h-[72px] max-w-[1200px] items-center gap-3 px-4 sm:px-8 lg:gap-8 lg:px-12">
        <button data-no-translate onClick={() => navigate("home")} aria-label={`Luminetis · ${dictionary.nav[0]}`} className="shrink-0 cursor-pointer border-0 bg-transparent p-0 text-ink">
          <span className="hidden sm:block"><Logo /></span>
          <span className="sm:hidden"><Logo compact /></span>
        </button>
        <nav aria-label="Primary" className="ml-auto hidden items-center gap-1 lg:flex">
          {pages.map((item, index) => (
            <button data-no-translate key={item} onClick={() => navigate(item)} aria-current={page === item ? "page" : undefined} className={`cursor-pointer rounded-md border-0 bg-transparent px-3 py-2 text-sm font-medium transition-colors hover:bg-card ${page === item ? "text-amber-text" : "text-ink"}`}>
              {dictionary.nav[index]}
            </button>
          ))}
        </nav>
        <button data-no-translate onClick={() => setMenuOpen(true)} aria-label={dictionary.menu} aria-expanded={menuOpen} className="ml-auto flex h-11 cursor-pointer items-center gap-2 rounded-lg border border-line bg-transparent px-3 font-mono text-[12.5px] uppercase tracking-[0.06em] lg:hidden">
          <Icon name="menu" className="text-[20px]" />
          <span className="hidden min-[430px]:inline">{dictionary.menu}</span>
        </button>
        <div data-no-translate role="group" aria-label={dictionary.theme} className="flex shrink-0 gap-1 rounded-lg border border-line p-1">
        ...
        <div data-no-translate ref={languageRef} className="relative shrink-0">
      ...
      {menuOpen && (
        <div className="fixed inset-0 top-0 z-40">
          ...
          <nav aria-label="Mobile" className="absolute right-0 top-0 flex h-dvh w-[min(88vw,360px)] flex-col gap-1 overflow-y-auto border-l border-line bg-page p-6">
            ...
            {pages.map((item, index) => (
              <button data-no-translate key={item} onClick={() => navigate(item)} aria-current={page === item ? "page" : undefined} className={`cursor-pointer border-0 border-b border-line bg-transparent px-3 py-3.5 text-left font-display text-xl font-medium ${page === item ? "text-amber-text" : "text-ink"}`}>
                {dictionary.nav[index]}
              </button>
            ))}
          </nav>
